<template>
  <div class="app-container">
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogUrlVisible" center modal show-close top="3vh" width="60%">
      <el-form ref="dataUrlForm" :model="temp" :rules="rules">
        <el-form-item :label-width="formLabelWidth" label="App名称:" prop="appChineseName">
          <el-input v-model="versionTemp.appName" disabled/>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="升级地址:" prop="appChineseName">
          <el-input v-model="versionTemp.updateUrl"/>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="版本号:" prop="appChineseName">
          <el-input v-model="versionTemp.appVersion"/>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="更新说明:" prop="appChineseName">
          <el-input v-model="versionTemp.updateMessage"/>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="跳转地址:" prop="appChineseName">
          <el-input v-model="versionTemp.jumpKey" placeholder="请输入内容">
            <template slot="prepend">https://d.fashioncomm.com/download/dl.html?g=</template>
          </el-input>
          <!-- <el-input v-model="versionTemp.jumpKey" size="5"/> -->
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="链接地址:" prop="appChineseName">
      ...
      <el-form ref="dataForm" :model="temp" :rules="rules">
        ...
        <el-form-item :label-width="formLabelWidth" label="支持设备:">
          <el-checkbox-group v-model="productId" size="small">
            <el-checkbox v-for="index in productList" :label="index.value" :key="index.val" border>{{ index.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        ...
        <div class="dialog-footer" align="right">
          ...
          <el-button v-else type="primary">编辑</el-button>
        </div>
      </el-form>
    </el-dialog>
    <el-table
      ...
      <el-table-column label="更新操作" min-width="160" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="handleOpen(scope.row,'ios')">IOS</el-button>
          <el-button size="small" type="primary" @click="handleOpen(scope.row,'Android')">Android</el-button>
        </template>
      </el-table-column>
...
import { getAPPList, addProduct, addApp, getListCustomer, getProductList, getWatchFaceList, getVersionById, addAppVersion } from '@/api/dealer'
...
export default {
  ...
  methods: {
    ...
    handleOpen(row, style) {
      ...
      const id = style === 'ios' ? row.ios : row.android
    ...
    handleAdd: function(params) {
      ...
      getProductList({ limit: 1000, start: 0 }).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.productList.push({ value: String(value1.productId), name: value1.appChineseName })
        })
      })
    ...
    updateData() {
      this.$refs['dataUrlForm'].validate((valid) => {
        if (valid) {
