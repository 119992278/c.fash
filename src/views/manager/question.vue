    ...
    <el-tabs v-model="activeName" type="border-card" @tab-click="handleTabClick">
      <el-tab-pane :label="$t('table.all')" name="all">
        <el-table
          ...
          <el-table-column :label="$t('table.question')" min-width="180" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.opiNion }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('table.answer')" min-width="180" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
...
export default {
  filters: {
    ...
    feedBackAnswerStatus(arr) {
      if (arr != null) {
        let answer = ''
        if (arr.length > 0) {
          answer = arr[arr.length - 1].answer
        }
        return answer
      } else {
        return ''
  ...
  methods: {
    ...
    handleUpdate: function(row) {
      ...
      this.answer = ''
      if (row.feedBackAnswer.length > 0) {
        this.answer = row.feedBackAnswer[row.feedBackAnswer.length - 1].answer
      }
