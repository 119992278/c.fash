...
export default {
  ...
  data() {
    return {
      ...
      ydata: [{ 'value': 1408, 'name': '60后' }, { 'value': 2841, 'name': '70后' }, { 'value': 6788, 'name': '80后' }, { 'value': 8933, 'name': '90后' }, { 'value': 4764, 'name': '60前' }, { 'value': 5367, 'name': '00后' }],
      chart: null
    }
  },
  created() {
  ...
  mounted() {
    this.initChart(this.xdata, [{ 'value': 0, 'name': '60后' }])
    setTimeout(() => {
      this.fetchData()
    }, 2000)
  },
  ...
  methods: {
    fetchData() {
      ...
      getCountAge({ beforeDays: null }).then(response => {
        ...
        console.log(JSON.stringify(_tempYdata))
