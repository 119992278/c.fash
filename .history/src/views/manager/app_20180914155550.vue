    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible" center modal show-close top="3vh" width="60%">
      <el-form ref="dataForm" :model="temp" :rules="rules">
        ...
        <el-form-item :label-width="formLabelWidth" label="公司中文名称:" prop="customerName">
          <el-select v-model="temp.customerName" filterable placeholder="请选择" @change="setcustomerCode">
        ...
        <el-form-item :label-width="formLabelWidth" label="支持设备:">
          <el-checkbox-group v-model="productId" size="medium" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="(item,index) in productList" :label="item.customerId" :key="index" border>{{ item.productName }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="支持表盘:">
          <el-checkbox-group v-model="watchFaceIdd" size="medium" @change="handleCheckedCitiesChange">
...
export default {
  ...
  methods: {
    ...
    handleAdd: function(params) {
      ...
      getProductList(this.temp).then(response => {
