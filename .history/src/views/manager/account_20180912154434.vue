    ...
    <el-dialog :visible.sync="dialogFormAuthority" title="授权管理" center modal show-close top="1vh" width="70%">
      <el-form ref="dataForm" :model="temp" :rules="rules">
        <el-form-item :label-width="formLabelWidth" label="公司编号:">
          <el-checkbox-group v-model="authority1" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="index in customeroptions" :label="index.value" :key="index.val">{{ index.label }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="授权APP:">
          <el-checkbox-group v-model="authority2" @change="handleCheckedCitiesChange">
        ...
        <el-form-item :label-width="formLabelWidth" label="授权固件:">
          <el-checkbox-group v-model="authority3" @change="handleCheckedCitiesChange">
    ...
    fetchData() {
      ...
      getListCustomer({}).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.customeroptions.push({ value: value1.customerCode, label: value1.customerName })
        })
      })
    },
    search(name) {
