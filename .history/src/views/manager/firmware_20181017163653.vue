  ...
  <div class="app-container">
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogHardwareFormVisible" center modal show-close top="3vh" width="80%">
          ...
          <el-table-column label="上传固件" align="center" min-width="100" show-overflow-tooltip>
            <template slot-scope="scope">
              <el-upload
                ref="upload"
                :headers="myHeaders"
                :on-success = "uploadhandleOnSuccess"
                :file-list="fileList"
                :auto-upload="true"
      ...
      <div class="dialog-footer" align="right">
        ...
        <el-button type="info" @click="addDialogHardwareForm">新增一行</el-button>
        <el-button type="primary" @click="updateDataHardwareForm">{{ $t('table.edit') }}</el-button>
      </div>
    </el-dialog>
...
import { getProductList, addProduct, getHardwareVersion, getproductInfo, editProductInfo, editProductVersionInfo, pullOffVersion } from '@/api/dealer'
...
export default {
  ...
  data() {
    return {
      ...
      gridData: [],
      tempGridData: { productId: '', versions: '' },
      temp: {
        customerId: null,
        shelveFlag: '',
        mainImg: '',
  ...
  methods: {
    ...
    addDialogHardwareForm() {
      this.gridData.push({
        'versionId': '', 'productId': this.gridData.productId, 'deviceVersion': '', 'buildVersion': '',
        'updateUrl': '',
        'updateMessage': null,
        'fileCrcSize': null,
        'invalidStatus': null,
        'insertTime': 1539762505000,
        'byforce': 1,
        'firmwareType': '',
        'md5': ''
      })
    },
    updateDataHardwareForm() {
      this.gridData.map(function(value, key, arr) {
        value.byforce = value.byforce === true ? 1 : 0
      })
      // this.tempGridData = { productId: 140, versions: this.gridData }
      this.tempGridData.versions = this.gridData
      editProductVersionInfo(this.tempGridData).then(response => {
        this.dialogHardwareFormVisible = false
        Message({
    ...
    handleHardwareForm(row) {
      getHardwareVersion(row.productId).then(response => {
        ...
        this.tempGridData = { productId: response.data.productId }
        this.gridData = response.resMap.list
        this.gridData.map(function(value, key, arr) {
          value.byforce = value.byforce === 1
        })
      })
    },
    handleOpen(row) {
    ...
    uploadhandleOnSuccess(file) {
      this.$nextTick(function() {
        this.gridData[this.gridData.length - 1].updateUrl = process.env.BASE_API + file.msg
        this.gridData[this.gridData.length - 1].md5 = file.data
      })
    }
  }
}
