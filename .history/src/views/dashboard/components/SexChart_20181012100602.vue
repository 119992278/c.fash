...
export default {
  ...
  data() {
    return {
      xdata: [i18n.t('chart.man'), i18n.t('chart.woman')],
      ydata: [{ 'value': 0, 'name': i18n.t('chart.man') }, { 'value': 0, 'name': i18n.t('chart.woman') }],
      chart: null
    }
  },
  created() {
        ...
        if (response.rows.length > 0) {
          ...
          response.rows.map(function(value, key, arr) {
            if (value.unitFormat === '0') {
              _tempYdata.push({ value: value.number, name: i18n.t('chart.man') })
            } else if (value.unitFormat === '1') {
              _tempYdata.push({ value: value.number, name: i18n.t('chart.woman') })
            }
          })
