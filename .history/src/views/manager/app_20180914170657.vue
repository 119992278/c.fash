<template>
  <div class="app-container">
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible" center modal show-close top="3vh" width="80%">
      <el-form ref="dataForm" :model="temp" :rules="rules">
        ...
        <el-form-item :label-width="formLabelWidth" label="支持设备:">
          <el-checkbox-group v-model="productId" size="medium">
        ...
        <el-form-item :label-width="formLabelWidth" label="支持表盘:">
          <el-checkbox-group v-model="watchFaceId" size="medium" >
    ...
    <el-table
      ...
      <el-table-column label="操作" min-width="160" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button size="mini" type="success" @click="handleModifyStatus(scope.row,'published')">编辑IOS</el-button>
          <el-button size="mini" type="success" @click="handleModifyStatus(scope.row,'published')">编辑Android</el-button>
          <</template>
      </el-table-column>
      <el-table-column label="发布新版本" min-width="180" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status!='published'" size="mini" type="success" @click="handleModifyStatus(scope.row,'published')">发布IOS</el-button>
          <el-button v-if="scope.row.status!='draft'" size="mini" type="primary" @click="handleModifyStatus(scope.row,'draft')">发布Android</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getAPPList, addProduct, addApp, getListCustomer, getProductList, getWatchFaceList } from '@/api/dealer'
