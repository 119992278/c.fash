...
import { getAccountAuthority, getAccountList, addDealer, editDealer, getDealerInfo, getListCustomer, queryAccountAuthority, getAPPList, getProductList } from '@/api/dealer'
  ...
  methods: {
    fetchData() {
      ...
      getListCustomer({}).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.customerList.push({ value: value1.customerId, label: value1.customerCode })
        })
      })
    },
    search(name) {
    ...
    handleAuthority: function(row) {
      ...
      getAccountAuthority(row.customerAccountId).then(response => {
        _this.authority1 = response.authority1.split(',')
        _this.authority2 = response.authority2.split(',')
        _this.authority3 = response.authority3.split(',')
      })
    },
