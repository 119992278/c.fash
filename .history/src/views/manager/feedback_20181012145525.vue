<template>
  <div class="app-container">
    ...
    <el-tabs v-model="activeName" type="border-card" @tab-click="handleTabClick">
      <el-tab-pane :label="$t('table.all')" name="all">
        <el-table
          ...
          <el-table-column :label="$t('table.operation')" width="100" align="center">
            <template slot-scope="scope">
              <el-button v-if="scope.row.feedBackAnswer === null" size="mini" type="success" @click="handleUpdate(scope.row)">答复</el-button>
      ...
      <el-tab-pane :label="$t('table.answered')" name="call">
        <el-table
          v-loading="listLoading"
          :data="list"
          element-loading-text="Loading"
          ...
          <el-table-column :label="$t('table.answerTime')" min-width="80" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              {{ scope.row.insterTime |momentStamp }}
            </template>
          </el-table-column>
          ...
          <el-table-column :label="$t('table.operation')" width="100" align="center">
            <template slot-scope="scope">
              <el-button v-if="scope.row.feedBackAnswer === null" size="mini" type="success" @click="handleUpdate(scope.row)">{{ $t('table.answer') }}</el-button>
              <el-button v-else size="mini" type="success" disabled>{{ $t('table.answered') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination :current-page="listQuery.sEcho" :page-sizes="[10,20,30,40,50,100]" :page-size="listQuery.limit" :total="totalQuantity" background layout="total, sizes, prev, pager, next, jumper" @size-change="handleSizeChange" @current-change="handleCurrentChange"/>
      </el-tab-pane>
      <el-tab-pane :label="$t('table.toBeAnswered')" name="nocall">
        <el-table
          v-loading="listLoading"
          :data="list"
          element-loading-text="Loading"
          ...
          <el-table-column l:label="$t('table.answer')" min-width="80" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.feedBackAnswer | feedBackAnswerStatus }}</span>
            </template>
          </el-table-column>
