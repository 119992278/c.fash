  ...
  <div class="app-container">
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible" center modal show-close top="1vh" width="70%">
      <el-form ref="dataForm" :model="temp" :rules="rules">
        ...
        <el-form-item v-if="temp.customerType=='2'" :label-width="formLabelWidth" :label="$t('route.data')">
          <el-checkbox-group v-model="authoList">
        ...
        <el-form-item v-if="temp.customerType=='2'" :label-width="formLabelWidth" :label="$t('route.extensionService')">
          <el-checkbox-group v-model="authoList">
            <el-checkbox label="13">{{ $t('route.EmailPromotion') }}</el-checkbox>
            <el-checkbox label="14">{{ $t('route.shortMessagePromotion') }}</el-checkbox>
            <el-checkbox label="15">{{ $t('route.appMessagePush') }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item v-if="temp.customerType=='2'" :label-width="formLabelWidth" :label="$t('route.settings')">
          <el-checkbox-group v-model="authoList">
            <el-checkbox label="16">{{ $t('route.appSetting') }}</el-checkbox>
            <el-checkbox label="17">{{ $t('route.hardwareSetting') }}</el-checkbox>
            <el-checkbox label="18">{{ $t('route.commonQuestionSetting') }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <div class="dialog-footer" align="right">
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.account')" min-width="130" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.customerAccount }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.type')" width="110" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{ scope.row.customerType | customerTypeFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.companyCode')" min-width="130" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.customerName }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.type')" min-width="110" align="center">
        <template slot-scope="scope">
          {{ scope.row.customerCode }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.contacter')" min-width="110" align="center">
        <template slot-scope="scope">
          {{ scope.row.contactUser }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.creatTime')" min-width="160" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.insertTime }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.status')" min-width="110" align="center">
        <template slot-scope="scope">
  ...
  methods: {
    ...
    fetchData() {
      ...
      getListCustomer({}).then(response => {
        _this.customerList = []
        response.rows.map(function(value1, index) {
    ...
    handleDialog: function(params) {
      this.dialogStatus = i18n.t('table.newAddAccount')
      this.temp.customerType = '2'
      this.temp.sex = '0'
      this.temp.authoList = []
      this.dialogFormVisible = true
      this.temp.customerName = '广东乐源数字技术有限公司'
      this.temp.customerCode = 'appscomm'
    },
    ...
    handleUpdate: function(row) {
      this.dialogStatus = i18n.t('table.editAccount')
