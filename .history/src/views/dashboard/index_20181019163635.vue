  ...
  methods: {
    async fetchData() {
      ...
      regUserResult.rows.map(function(value, key) {
        this.lineChartUserData[parseInt(value.unitFormat) - 1] = parseInt(value.number)
      })
      ...
      this.xdata.map(function(value, key) {
        if (key < dayD) {
          this.lineChartUserData[key] = this.lineChartUserData[key] === undefined ? 0 : this.lineChartUserData[key]
