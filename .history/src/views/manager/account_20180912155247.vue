<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogFormAuthority" title="授权管理" center modal show-close top="1vh" width="70%">
      <el-form ref="dataForm" :model="temp" :rules="rules">
        <el-form-item :label-width="formLabelWidth" label="公司编号:">
          <el-checkbox-group v-model="authority1" size="small" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="index in customerList" :label="index.value" :key="index.val" border>{{ index.label }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="授权APP:">
          <el-checkbox-group v-model="authority2" size="small" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="index in appList" :label="index.value" :key="index.val">{{ index.label }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="授权固件:">
          <el-checkbox-group v-model="authority3" size="small" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="index in productList" :label="index.value" :key="index.val">{{ index.label }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    <el-table
      ...
      <el-table-column label="操作" min-width="180" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status!='draft'" size="mini" type="primary" @click="handleAuthority(scope.row)">授权</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getAccountList, addDealer, editDealer, getDealerInfo, getListCustomer, queryAccountAuthority, getAPPList, getProductList } from '@/api/dealer'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        customerAccount: validNoEmpty,
        customerEamil: validNoEmpty,
        customerContact: validNoEmpty,
        customerAddress: validNoEmpty,
        customerPassword: validNoEmpty,
        customerName: validNoEmpty,
        customerType: validNoEmpty,
        contactUser: validNoEmpty,
        customerCode: validNoEmpty
      }
    }
  },
  watch: {
  ...
  methods: {
    fetchData() {
      ...
      getListCustomer({}).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.customerList.push({ value: value1.customerCode, label: value1.customerCode })
        })
      })
    },
    search(name) {
    ...
    handleAuthority: function(row) {
      ...
      getAPPList({ limit: 1000, start: 0 }).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.appList.push({ value: value1.appId, label: value1.appChineseName })
        })
      })
      getProductList({ limit: 1000, start: 0 }).then(response => {
        response.rows.map(function(value1, index, arr) {
          _this.productList.push({ value: value1.productId, label: value1.productCode })
        })
      })
