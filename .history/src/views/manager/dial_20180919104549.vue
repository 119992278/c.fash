...
import { getWatchFacelist, addProduct, delWatchFaceById, getwatchFaceById, editWatchFace } from '@/api/dealer'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      delWatchFaceById(row.watchFaceId).then(response => {})
    },
    ...
    updateData: function() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.watchFaceUrl = this.temp.watchFaceUrl.split('base64,')[1]
