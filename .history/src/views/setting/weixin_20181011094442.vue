<template>
  <div class="weixin-container">
    <el-form ref="dataForm" :rules="rules" >
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item v-if="results.length > 0" :label-width="formLabelWidth" label="授权结果:">
            <el-tag
...
export default {
  ...
  data() {
    return {
      ...
      product: '38143',
      macs: ['123456789ABC1', '123456789ABC2', '123456789ABC3'],
      results: [],
      inputVisible: false,
      inputValue: '',
      loading: false,
  ...
  methods: {
    ...
    accredit() {
      ...
      const _this = this
      axios
        .post('http://localhost:3000/accredit', { appid: this.appid, secret: this.secret, product: this.product, macs: this.macs })
        .then(function(response) {
          _this.loading = false
          const resp = response.data.resp
          resp.map(function(value, key, arr) {
            _this.results.push(_this.macs[key] + '(' + value.errmsg + ')')
          })
        })
