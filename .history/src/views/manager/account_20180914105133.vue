  ...
  <div class="app-container">
      ...
      <el-form ref="dataForm" :model="temp" :rules="rules">
        <el-form-item :label-width="formLabelWidth" label="公司编号:">
          <el-checkbox-group v-model="authority1" size="medium" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="(item,index) in customerList" :label="index.value" :key="index.value" border>{{ index.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-table
      ...
      <el-table-column label="操作" min-width="180" align="center">
        <template slot-scope="scope">
          <el-button :disabled="!$store.state.user.authority.authority1.includes(String(scope.row.customerId))" size="mini" type="success" @click="handleUpdate(scope.row)">编辑</el-button>
          <el-button v-if="$store.getters.customerId === 0" size="mini" type="primary" @click="handleAuthority(scope.row)">授权</el-button>
        </template>
      </el-table-column>
    </el-table>
