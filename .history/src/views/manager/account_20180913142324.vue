  ...
  <div class="app-container">
      ...
      <el-form ref="dataForm" :model="temp" :rules="rules">
        <el-form-item :label-width="formLabelWidth" label="公司编号:">
          <el-checkbox-group v-model="authority1" size="medium" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="index in customerList" :label="index.value" :key="index.value" border>{{ index.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      ...
      <el-form ref="dataForm" :model="temp" :rules="rules">
        ...
        <el-form-item :label-width="formLabelWidth" label="公司中文名称:" prop="customerName">
          ...
          <el-select v-model="temp.customerName" :disabled="dialogStatus === '编辑账户'" filterable placeholder="请选择" @change="setcustomerCode">
            <el-option v-for="item in customerList" :key="item.value" :label="item.name" :value="item.code" />
          </el-select>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="公司编号:">
    ...
    <el-table
      ...
      <el-table-column label="操作" min-width="180" align="left">
        <template slot-scope="scope">
          <el-button :disabled="userAuthority1.includes(scope.row.customerId)" size="mini" type="success" @click="handleUpdate(scope.row)">编辑</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      userAuthority1: this.$store.state.user.authority.authority1,
  ...
  created() {
    this.fetchData()
    console.log(this.userAuthority1.includes(1))
  },
  mounted() {
  },
  methods: {
